...
const slackToken = 'xoxb-1395688986436-1436510679728-75PPYPdl3DKRVpwC7dK39GAo'; 
const app = Express();

app.use(BodyParser.urlencoded({ extended: true }));
app.use(BodyParser.json());

// 發送訊息到 Slack 裡面的 function
async function sendToSlack(text) {
    const url = 'https://slack.com/api/chat.postMessage';
    const res = await axios.post(url, {
        channel: '#home', //自己設定的 Slack Channel
        text: text,
        attachments: [
            {
                "text": "請進行重要性評估",
...
// 填完問卷後，會打這隻 API，我們會從 req 中拿到 svid 和 hash，再將資料組合成 Webhook Query API ，取得答案，再使用官方的解密方式，解密出 json 格式的資料，再放資料後打 Slack 通知的 API
app.post('/surveywebhook', function (req) {
    ...
    fetch(url)
        ...
        .then((dat) => {
            ...
            sendToSlack(obj.result[4].answer);
